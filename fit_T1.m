...
%     x = x/1000;
    ...
    switch fitfunc
        case "Exp1"
            ...
            yfit = curve(x);
        case "Exp2"
            % The double expenontial fit in exponfit is bad
            fo = fitoptions(func,"StartPoint",[c c(2) k k]);
            ft = fittype(func,"options",fo);
            [curve, gof] = fit(x,y,ft);
    ...
    labels{2*i-1} = strcat(num2str(temperature(i))," K");
    hold on
