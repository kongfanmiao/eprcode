...
%                   - {'Exp1', 'Exp2', 'StrExp'}
%       FitModulation
%                   fit the Tm decay with modulation or not
%       TwoOmega    fit with omega and 2*omega
...
switch fitfunc
    case "Exp1"
        if ~args.FitModulation
            FitResult = table( ...
                'VariableNames',{'Temperature', 'Tm', 'R-Squared', 'TmConfInt_lower', 'TmConfInt_upper'}, ...
                'Size', [numFiles, 5], ...
            ...
            if ~args.TwoOmega
                FitResult = table( ...
                    'VariableNames',{'Temperature', 'Tm', 'C_mod', ...
                        'Modulation Frequency', 'R-Squared', 'TmConfInt_lower', 'TmConfInt_upper'}, ...
                    'Size', [numFiles, 7], ...
    ...
    case "Exp2" % confidence interval doesn't work for Exp2
        if ~args.FitModulation
    ...
    case "StrExp"
        if ~args.FitModulation
            FitResult = table('VariableNames', ...
                {'Temperature', 'Tm', 'Stretch Factor', 'R-Squared', 'TmConfInt_lower', 'TmConfInt_upper'}, ...
            'Size', [numFiles, 6], ...
            'VariableTypes',repelem({'double'},6));
            FitResult.Properties.VariableUnits = {'K', timeUnit, '', '','',''};
        else
            if ~args.TwoOmega
                FitResult = table('VariableNames', ...
                    ...
                    'C_mod', 'Modulation Frequency', 'R-Squared', 'TmConfInt_lower', 'TmConfInt_upper'}, ...
                    'Size', [numFiles, 8], ...
                    'VariableTypes',repelem({'double'},8));
                FitResult.Properties.VariableUnits = ...
                    {'K', timeUnit, '', '', 'MHz', '','',''};
                ...
                FitResult = table( ...
                    'VariableNames', ...
                        ...
                        'C_mod2', 'Modulation Frequency', 'R-Squared', 'TmConfInt_lower', 'TmConfInt_upper'}, ...
                    'Size', [numFiles, 9], ...
                    'VariableTypes',repelem({'double'},9));
                FitResult.Properties.VariableUnits = ...
                    {'K', timeUnit, '', '', '', 'MHz', '','',''};
...
for i = 1:numFiles
    ...
    switch fitfunc
        case "Exp1"
            ...
            ci = confint(curve);
            FitResult.TmConfInt_lower(i) = ci(1,3);
            FitResult.TmConfInt_upper(i) = ci(2,3);

        case "Exp2" % confidence interval doesn't work for Exp2
            if ~args.FitModulation
                func = @(c1, c2, rc, k1, rk, x) ...
                        c1 + c2*exp(-x/k1) + c2*rc*exp(-x/(k1*rk));
                bounds = [c(1)/2,c(2)/100,rc0,0.001/k,rk0;...
                          c(1)*2,c(2)*100,rc1,1000/k,rk1];
                startPoint = [c(1), c(2), 1, 1/k, 1];
                [curve, gof] = do_fit(x, y, func, startPoint, bounds);
            else
                if ~args.TwoOmega
                    func = @(c1, c2, rc, k1, rk, c4, omega, phi, x) ...
                            c1 + (c2*exp(-x/k1) + c2*rc*exp(-x/(k1*rk))).*( ...
                            1 + c4*sin(omega*x+phi));
                    bounds = [c(1)/2,c(2)/10,rc0,0.001/k,rk0, ...
                                                1e-3,omegaMin,-inf;
                              c(1)*2,c(2)*1000,rc1,1000/k,rk1, ...
                                                100,omegaMax,inf];
                    startPoint = [c(1), c(2), 1, 1/k, 1, 1, omega, 0];
                    [curve, gof] = do_fit(x, y, func, startPoint, bounds);
                    FitResult.C_mod(i) = curve.c4;
                else
                    func = @(c1, c2, rc, k1, rk, c4, c5, omega, phi_1, ...
                            phi_2, x) ...
                            c1 + (c2*exp(-x/k1) + c2*rc*exp(-x/(k1*rk))).*(1 + ...
                    ...
                    startPoint = [c(1), c(2), 1, 1/k, 1, 1, 1, ...
                                            omega, 0, 0];
                    [curve, gof] = do_fit(x, y, func, startPoint, bounds);
            ...
            [T_long, T_longPerc, T_short, T_shortPerc] = ...
                get_Tlong_and_Tshort([curve.k1, curve.rk*curve.k1], ...
                [curve.c1, curve.c2, curve.c2*curve.rc]);
        ...
        case "StrExp"
            ...
            FitResult.Tm(i) = curve.k;
            ci = confint(curve);
            FitResult.TmConfInt_lower(i) = ci(1,4);
            FitResult.TmConfInt_upper(i) = ci(2,4);
